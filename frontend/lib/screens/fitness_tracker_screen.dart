...
class _FitnessTrackerScreenState extends State<FitnessTrackerScreen>
  ...
  Widget build(BuildContext context) {
    return Consumer2<FitnessService, FirebaseService>(
      builder: (context, svc, fbSvc, _) {
        // calories consumed comes from Firebase meal logs
        final caloriesConsumed = fbSvc.totalCaloriesToday;
        final calorieGoal = fbSvc.calorieGoal;
              ...
              children: [
                ...
                if (!svc.pedometerAvailable)
                  Container(
                    ...
                    child: Row(children: [
                      ...
                      Expanded(
                        child: Text(
                          'Initialising step sensor...',
                          style: GoogleFonts.poppins(
                              color: AppTheme.yellow, fontSize: 11),
                        ),
                      ),
                    ]),
                  ),
                ...
                Row(children: [
                  Expanded(child: _LiveCard(
                      ...
                      value: svc.caloriesBurned.toStringAsFixed(0),
                      unit: 'kcal')),
                  const SizedBox(width: 10),
                  Expanded(child: _LiveCard(
                      ...
                      color: AppTheme.teal,
                      label: 'Distance',
                      value: svc.distanceKm.toStringAsFixed(2),
                      unit: 'km')),
                  const SizedBox(width: 10),
                  Expanded(child: _LiveCard(
                  ...
                  child: LineChart(LineChartData(
                    ...
                    gridData: FlGridData(
                      ...
                      getDrawingHorizontalLine: (_) => FlLine(
                          color: AppTheme.darkCardLight, strokeWidth: 1),
                    ),
                    borderData: FlBorderData(show: false),
                    lineBarsData: [
                      // Goal dashed line
                ...
                // ── Calorie balance — uses Firebase meals ──────────────────
                Container(
                  padding: const EdgeInsets.all(16),
                  decoration: BoxDecoration(
                      color: AppTheme.darkCard,
                  ...
                  child: Column(
                    ...
                    Row(
                      ...
                      _CalorieItem(
                          label: 'Burned',
                          value: '${svc.caloriesBurned.toInt()}',
                          color: AppTheme.teal),
                      Text('=',
                          style: GoogleFonts.poppins(
                              color: Colors.white38, fontSize: 22)),
                      _CalorieItem(
                        label: 'Net',
                        value:
                            '${(caloriesConsumed - svc.caloriesBurned).toInt()}',
                        color: (caloriesConsumed - svc.caloriesBurned) < 600
                            ? AppTheme.green
                            : AppTheme.accent,
                      ),
                    ]),
                    ...
                    ClipRRect(
                      ...
                      child: LinearProgressIndicator(
                        value: calorieGoal > 0
                            ? (svc.caloriesBurned / calorieGoal)
                                .clamp(0.0, 1.0)
                            : 0,
                        backgroundColor: AppTheme.darkCardLight,
                        valueColor:
                    ...
                    Text(
                      calorieGoal > 0
                          ? '${((svc.caloriesBurned / calorieGoal) * 100).toStringAsFixed(1)}% of ${calorieGoal.toInt()} kcal goal burned'
